...
public class CustomListTest {
    ...
    public static void runAllTests() {
        ...
        runTest(
                ...
                CustomListTest::shouldAddAndGet
        );

        runTest(
                "Вставка и замена",
                CustomListTest::shouldInsertAndSet
        );

        runTest(
                "Удаление",
                CustomListTest::shouldRemove
        );

        runTest(
                "Поиск элементов",
                CustomListTest::shouldFindElements
        );

        runTest(
                "Итератор",
                CustomListTest::shouldIterate
        );

        runTest(
                "Добавление коллекции",
                CustomListTest::shouldAddAll
        );

        runTest(
                "Массив и Stream API",
                CustomListTest::shouldSupportArrayAndStream
        );

        runTest(
                "Очистка",
                CustomListTest::shouldClear
        );

        runTest(
                "Проверка индексов",
                CustomListTest::shouldCheckIndexes
        );
    ...
    private static void shouldInsertAndSet() {
        ...
        checkEquals(
                List.of("A", "B", "C"),
                list,
                "Неверный порядок после вставки"
        );
    ...
    private static void shouldRemove() {
        ...
        checkEquals(
                List.of("A", "C", "D"),
                list,
                "Неверный список после удаления"
        );
    ...
    private static void shouldSupportArrayAndStream() {
        ...
        check(
                Arrays.equals(
                        new Integer[]{5, 1, 4, 2, 3},
                        array
                ),
                "toArray работает неправильно"
        );
